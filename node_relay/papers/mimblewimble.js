module.exports = {
   'Introduction': [
      ...
      "I call my creation _PROTOCOL_ because it is used to prevent the blockchain from talking about all user's information.",
   ],
...
4. Recipient attaches signature with k to the transaction, and the explicit fee. It has done."},
      ...
      {'code': "1. Explicit amounts for new money (block subsidy or sidechain peg-ins) with whatever else data this needs. For a sidechain peg-in maybe it references a Bitcoin transaction that commits to a specific excess k*G value?\n\
\n\
...
4. Excess k*G values for all transactions"},
   ...
   'Merging Transactions Across Blocks': [
      ...
      {'code': "1. Explicit amounts for new money (block subsidy or sidechain peg-ins) with whatever else data this needs.\n\
\n\
...
An exception is that if the outputs are amount zero, it is possible to make two that are negatives of each other, and the pair can be revived without anything breaks. So to prevent consensus problems, outputs 0-amount should be banned. Just add H at each output, now they all amount to at least 1."}
   ...
   'Future Research': [
      ...
      {'code': "2. We require user to check all k*G values, when in fact all that is needed is that their sum is of the form k*G. Instead of using signatures is there another proof of discrete logarithm that could be combined?"},
      {'code': "3. There is a denial-of-service option when a user downloads the chain, the peer can give gigabytes of data and list the wrong unspent outputs. The user will see that the result do not add up to 0, but cannot tell where the problem is.\n\
\n\
